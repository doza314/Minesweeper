...
inline std::string getResourcePath(const std::string& file) {
    CFBundleRef mainBundle = CFBundleGetMainBundle();
    CFStringRef cfFile = CFStringCreateWithCString(NULL, file.c_str(), kCFStringEncodingUTF8);
    CFURLRef fileURL = CFBundleCopyResourceURL(mainBundle, cfFile, NULL, NULL);
    char path[PATH_MAX];
    std::string result;

    if (fileURL && CFURLGetFileSystemRepresentation(fileURL, true, (UInt8*)path, PATH_MAX)) {
        result = std::string(path);
    }

    if (cfFile) CFRelease(cfFile);
